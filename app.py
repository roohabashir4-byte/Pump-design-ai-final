...
def apply_pending_missing_input_updates() -> None:
    """Apply values entered in the missing-input panel before widgets are created."""
    pending = st.session_state.pop("pending_missing_inputs", None)
    if not pending:
        return
    for key, value in pending.items():
        if isinstance(value, dict):
            if value.get("value") is not None:
                st.session_state[f"v_{key}"] = float(value["value"])
            if value.get("unit") is not None:
                st.session_state[f"u_{key}"] = value["unit"]
    st.session_state["run_design_after_missing_input"] = True


def render_missing_input_panel(application_code: str) -> None:
    """Allow the engineer to provide required values without returning to the top of the form."""
    state = st.session_state.get("last_agent_state")
    if not state:
        return

    results = state.last_tool_results or []
    deterministic = next(
        (x.get("result") for x in reversed(results)
         if isinstance(x.get("result"), dict) and x["result"].get("status") == "MISSING_INPUT"),
        None,
    )
    ...
    missing = []
    for item in deterministic.get("calculations", []):
        if isinstance(item, dict) and item.get("status") == "MISSING_INPUT":
            missing.append(item)
    pipe = deterministic.get("pipe_sizing") or {}
    for name, item in pipe.items():
        if isinstance(item, dict) and item.get("status") == "MISSING_INPUT":
            missing.append({"name": f"{name.title()} pipe", "message": item.get("message")})

    if not missing:
        return

    st.divider()
    st.subheader("Provide Missing Input")
    st.info("The calculation stopped because the following engineering input is required. Enter it below and rerun the design. PumpDesign AI will not assume the value.")

    pending: dict[str, dict[str, Any]] = {}

    if application_code == "TRANSFER":
        names = {str(x.get("name", "")) for x in missing}
        if "Transfer design flow" in names:
            c1, c2 = st.columns([2.2, 1])
            value = c1.number_input("Required Transfer Time", min_value=0.0001, value=30.0, key="missing_transfer_time")
            unit = c2.selectbox("Unit", ["min", "h", "s"], key="missing_transfer_time_unit")
            pending["required_transfer_time"] = {"value": value, "unit": unit}

        if any("Static head" in n for n in names):
            st.caption("UGT and OHT design water levels are required for static head. Enter them in the main form above.")

    if missing:
        with st.expander("Why is this required?", expanded=False):
            for item in missing:
                if item.get("message"):
                    st.write(f"**{item.get('name', 'Required input')}:** {item['message']}")

    if pending and st.button("↻ Update Input & Rerun Design", type="primary", key="update_missing_input"):
        st.session_state["pending_missing_inputs"] = pending
        st.rerun()
...
apply_pending_missing_input_updates()
...
st.divider()
run = st.button("🚀 Run Pump Design", type="primary", use_container_width=True) or st.session_state.pop("run_design_after_missing_input", False)
...
if st.session_state.get("last_answer"):
    render_missing_input_panel(application_code)
    st.divider()
