"""Deterministic transfer-pump engineering workflow.

The workflow owns the numerical calculation. The LLM does not invent
engineering inputs or numerical results.
"""
...
def run_transfer_design(
    ...
    *,
    ...
    Expected frozen fields are accepted in a unit-aware form. Each numeric
    field may be either a raw number plus a sibling ``*_unit`` field, or
    ``{"value": ..., "unit": ...}``.
    """
    ...
    # IMPORTANT:
    # The current Streamlit UI sends "demand_basis".
    # Keep the deterministic workflow aligned with that field.
    demand_method = _value(data, "demand_basis")
    ...
    if (
        demand_method == "POPULATION_PER_CAPITA"
        and not _missing(population, per_capita)
    ):
        pop_value = (
            population["value"]
    ...
    if oht_capacity is None or transfer_time is None:
        out["calculations"].append(
            _result(
                "Transfer design flow",
                "MISSING_INPUT",
                formula="Q = V / t",
                message=(
                    "OHT capacity and required transfer time are "
                    "required for the full-fill transfer basis."
                ),
            )
        )
    ...
    volume_gal = (
        oht_capacity["value"]
        if isinstance(oht_capacity, dict)
        else oht_capacity
    )

    time_min = (
        transfer_time["value"]
        if isinstance(transfer_time, dict)
        else transfer_time
    )
    ...
    out["calculations"].append(
        _result(
            ...
            q.method,
            assumptions=[
                "Transfer volume is based on the finalized full-fill OHT capacity basis."
            ],
            references=refs,
        )
    )
    ...
    if _missing(
        ...
        out["calculations"].append(
            _result(
                ...
                formula="Hs = Z2 - Z1",
                message=(
                    "UGT and OHT design water levels are required."
                ),
            )
        )
        ...
        z1 = (
            ugt_level["value"]
            if isinstance(ugt_level, dict)
            else ugt_level
        )

        z2 = (
            oht_level["value"]
            if isinstance(oht_level, dict)
            else oht_level
        )

        if (
            isinstance(ugt_level, dict)
            and ugt_level.get("unit") == "ft"
        ):
            z1 = ft_to_m(z1)

        if (
            isinstance(oht_level, dict)
            and oht_level.get("unit") == "ft"
        ):
            z2 = ft_to_m(z2)
    ...
    suction_length = _value(
        data,
        "suction_pipe_length",
    )

    delivery_length = _value(
        data,
    ...
    if material is None:
        ...
        out["pipe_sizing"]["message"] = (
            "Pipe material is required."
        )
        out["status"] = "MISSING_INPUT"

    else:
        for name, length in (
            ("suction", suction_length),
            ("delivery", delivery_length),
        ):
            ...
            if length is None:
                out["pipe_sizing"][name] = {
                    ...
                    "message": (
                        f"{name.title()} pipe length is required."
                    ),
                }
            ...
            if (
                isinstance(length, dict)
                and length.get("unit") == "ft"
            ):
                length_value = ft_to_m(
                    length_value
                )

            min_v = (
                criteria.get("min_mps")
                if isinstance(criteria, dict)
                else None
            )

            max_v = (
                criteria.get("max_mps")
                if isinstance(criteria, dict)
                else None
            )
            ...
            selected = select_first_acceptable(
                candidates
            )
    ...
    # TDH can be completed only from the available loss terms.
    # Minor losses are deliberately not invented because fittings/valves
    # were not finalized inputs.

    if all(
        c.get("status") == "CALCULATED"
        ...
        out["calculations"].append(
            _result(
                ...
                "m",
                formula=(
                    "H_pump = H_static + h_major + h_minor"
                ),
                inputs={
                    "static_head_m": sh,
                    "major_losses_m": total_loss,
                    "minor_losses_m": None,
                },
                method=(
                    "Energy equation; available major-loss terms only"
                ),
                assumptions=[
                    "Minor losses are not fabricated because fittings/valves were not finalized inputs."
                ],
                references=refs,
                message=(
                    "TDH is provisional until applicable minor losses "
                    "are supplied or otherwise deterministically derived."
                ),
            )
        )
        ...
        out["validation"].append(
            {
                ...
                "severity": "WARNING",
                "message": (
                    "Minor losses are not included because no "
                    "fitting/valve inputs are available."
                ),
            }
        )
    ...
    out["design_basis"] = {
        ...
        "total_design_demand_supplied": (
            total_demand is not None
        ),
        "population_supplied": (
            population is not None
        ),
        "per_capita_supplied": (
            per_capita is not None
        ),
